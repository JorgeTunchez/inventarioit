        ...
        while( $arrTMP = each($_POST) ){
          ...
          if( $arrExplode[0] == "hdnIdSW"){
            ...
            if( $strAccionS == "A" ){
                ...
                $this->objModel->updateExpDetail($intSoftware, $intIDExpediente, $intComponenteS, "", $strValorS, $intPagoS, $intVersionS, "", "", "", 0, "","", $this->arrRolUser["ID"]);
            }
          }
...
class expediente_view{
    ...
    public function drawSelectArea(){
        ...
        ?>
        <label><h5>1. Seleccione un área:</h5></label>
    ...
    public function drawSelectColaborador($intArea = 0){
        ...
        ?>
        <label><h5>2. Seleccione un colaborador:</h5></label>
    ...
    public function drawContent(){
        ...
        <script>
            ...
            function agregarHD(){
                ...
                var $td = $("<td data-title='Nombre' style='text-align:center;'><select class='form-control' id='selectComponenteHD_"+max+"' name='selectComponenteHD_"+max+"' style='text-align: center;'><?php $arrComponenteHD = $this->objModel->getComponenteHD();reset($arrComponenteHD);while( $rTMP = each($arrComponenteHD) ){?><option value='<?php print $rTMP["key"];?>'><?php print $rTMP["value"]["NOMBRE"];?></option><?php } ?></select></td>");
                $tr.append($td);
            ...
            function agregarPL(){
                ...
                var $td = $("<td data-title='Nombre' style='text-align:center;'><select class='form-control' id='selectComponentePL_"+max+"' name='selectComponentePL_"+max+"' style='text-align: center;'><?php $arrComponentePL = $this->objModel->getComponentePL();reset($arrComponentePL);while( $rTMP = each($arrComponentePL) ){?><option value='<?php print $rTMP["key"];?>'><?php print $rTMP["value"]["NOMBRE"];?></option><?php } ?></select></td>");
                $tr.append($td);
            ...
            function agregarSW(){
                ...
                var $td = $("<td data-title='Nombre' style='text-align:center;'><select class='form-control' id='selectComponenteSW_"+max+"' name='selectComponenteSW_"+max+"' style='text-align: center;'><?php $arrComponenteSW = $this->objModel->getComponenteSW();reset($arrComponenteSW);while( $rTMP = each($arrComponenteSW) ){?><option value='<?php print $rTMP["key"];?>'><?php print $rTMP["value"]["NOMBRE"];?></option><?php } ?></select></td>");
                $tr.append($td);
